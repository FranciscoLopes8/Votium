...
contract VotoEstatisticas {
    ...
    modifier apenasAdmin() {
        require(msg.sender == admin, "Apenas o admin pode ver as estatisticas");
        _;
    }
    ...
    function obterVotosPorCandidato(
        uint _candidatoId
    ) public view apenasAdmin returns (uint) {
        return votosPorCandidato[_candidatoId];
    }

    function obterTotalVotos() public view apenasAdmin returns (uint) {
        return totalVotos;
    }
}
